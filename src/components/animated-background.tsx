...
const AnimatedBackground = () => {
  ...
  useEffect(() => {
    ...
    if (splineApp.getVariable("heading")) splineApp.setVariable("heading", selectedSkill.label);
    if (splineApp.getVariable("desc")) splineApp.setVariable("desc", selectedSkill.shortDescription);
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      if (activeSection === "skills") {
        ...
        if (splineApp.getVariable("heading")) splineApp.setVariable("heading", "");
        if (splineApp.getVariable("desc")) splineApp.setVariable("desc", "");
      }
  ...
  const handleSplineInteractions = () => {
    ...
    splineApp.addEventListener("keyDown", (e) => {
      ...
      if (skill) {
        setSelectedSkill(skill);
        if (splineApp.getVariable("heading")) splineApp.setVariable("heading", skill.label);
        if (splineApp.getVariable("desc")) splineApp.setVariable("desc", skill.shortDescription);
      }
    });
  ...
  return (
    <>
      ...
      <AnimatePresence>
        {/* Robust HTML-based Skill Tooltip (Fallback for missing Spline variables) */}
        {selectedSkill && (
          <div className="fixed bottom-12 left-1/2 -translate-x-1/2 z-[100] pointer-events-none w-full max-w-xl px-4">
            <motion.div 
              initial={{ opacity: 0, y: 20, scale: 0.95 }}
              animate={{ opacity: 1, y: 0, scale: 1 }}
              exit={{ opacity: 0, y: 20, scale: 0.95 }}
              className="bg-black/60 dark:bg-black/80 backdrop-blur-2xl border border-white/10 p-6 rounded-3xl shadow-[0_0_50px_rgba(0,0,0,0.5)] flex items-center gap-6"
            >
               <div className="w-16 h-16 shrink-0 bg-white/5 p-3 rounded-2xl border border-white/10">
                 <img src={selectedSkill.icon} alt={selectedSkill.label} className="w-full h-full object-contain" />
               </div>
